...
- **Analytics** — every algorithm benchmarked at training time (accuracy,
  precision, recall, **F1 score**, ROC AUC) plus the full set of
  diagnostic plots (model comparison, F1 ranking, confusion matrix, ROC
  curves, feature importance, class balance, correlation).
...
- **Data:** 500 real survey respondents, augmented with 1,500 synthetic
  respondents (bootstrapped from a real row with small bounded noise
  added to the numeric answers) for 2,000 rows total. The augmented
  dataset is saved to `dataset/mental_health_prediction_augmented.csv`.
- **Pipeline:** cleaning → synthetic augmentation → feature engineering
  (Wellbeing Index, Pressure Index) → encoding (label + one-hot) →
  correlation-based feature selection → standard scaling →
  respondent-grouped, stratified train/test split (so a synthetic row
  never leaks across the split from the real respondent it was derived
  from) → SMOTE-style class balancing on the training split only →
  comparison across 7 classifiers → grid-search tuning → final model
  selection.
- **Final model:** Random Forest — see the Analytics page in the running
  app for the full per-algorithm comparison (including F1 score) and
  diagnostic plots, or the About page for headline metrics.
- Full methodology and code lives in `train_pipeline.py` at the project
  root, which regenerates `model/model.pkl`, `model/preprocessing.pkl`,
  `model/analytics.json`, the augmented dataset, and the plots in
  `static/images/plots/` from `dataset/mental_health_prediction.csv`.
...
│   ├── mental_health_prediction_augmented.csv   # real + synthetic
│   └── prediction_history.csv
├── static/
│   ├── css/style.css
│   ├── js/main.js
│   └── images/
│       └── plots/            # generated by train_pipeline.py
├── templates/
│   ├── base.html
│   ├── home.html
│   ├── predict.html
